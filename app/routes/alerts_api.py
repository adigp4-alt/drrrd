...
@bp.route("/api/alerts/test-discord", methods=["POST"])
def test_discord():
    ok = send_discord("Test alert from Iran Investment Tracker")
    if ok:
        return jsonify({"status": "sent"})
    return jsonify({"error": "Failed. Check DISCORD_BOT_TOKEN and DISCORD_CHANNEL_ID env vars"}), 400
